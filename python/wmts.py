...
uniquement, tileInfo absent, GetCapabilities WMTS -> HTTP 400). La classe
WMTS ci-dessous ne peut donc plus etre instanciee : WebMapTileService(...)
echoue des la construction, puisqu'elle depend d'un GetCapabilities live.

Utiliser `wms.py` (classe `WMS`) a la place, qui offre la meme interface
publique (tile_to_xy, xy_to_tile, wgs_to_tile, get_tile, tile_width,
tile_height, ...) en s'appuyant sur GetMap WMS et des constantes de
tuilage figees, relevees sur un instantane archive des capabilities
WMTS d'origine.

Conserve ici a titre documentaire / historique.
